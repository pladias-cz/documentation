...
SELECT
    ...
    (
        ...
        WHERE st_intersects(records.coords_wgs, quadrants_full.geom_wgs) LIMIT 1) AS kvadrant,
...
## occurrence status including aggregated descendant taxa (parent_id is the taxon to be reported, inlcuding records linked to itself + all descendant taxa)
```sql
