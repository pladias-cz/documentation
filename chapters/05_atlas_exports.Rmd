...
SELECT
    records.id AS id,
    taxons.name_lat AS jmeno_stand,
    ...
    (
        SELECT STRING_AGG(quadrants_full.code, '; ')
        FROM atlas.records_quadrants
            INNER JOIN geodata.quadrants_full ON quadrants_full.id = records_quadrants.quadrants_id
        WHERE records_id = records.id) AS kvadrant,
...
FROM atlas.records
    LEFT JOIN public.taxons on records.taxon_id = taxons.id
