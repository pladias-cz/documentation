...
3) cleanup & truncate records table:
    ...
    ```
4) upload occurrences from Simple on megastroj, import records.csv via admin.pladias.cz Console ``` docker cp ./data/records.csv adminpladiascz-app-1:/srv/temp/data/records.csv   &&  nohup docker exec adminpladiascz-app-1 bin/console gbif:importRecords > output.log 2>&1 &```. When testing on localhost inside container, use ```NETTE_ENV=prod bin/console gbif:importRecords```
